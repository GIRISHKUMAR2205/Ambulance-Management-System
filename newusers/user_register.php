...
if($_SERVER["REQUEST_METHOD"]=="POST" && isset($_POST['submit'])){
    ...
    $query->store_result();
        if($query->num_rows>0){
           ...
           if(empty($error)){
               $insertQuery=$db->prepare("INSERT INTO newusers (name,Email,phonenu,username,Password) VALUES(  ?, ?,?,?,?);");
               $insertQuery->bind_param("sssss",$name,$Email,$phonenu,$username,$password_hash);
