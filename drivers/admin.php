...
$sql="select * from patients WHERE status='ASSIGNED'";
$res=mysqli_query($db,$sql);
if($res){
while($row = mysqli_fetch_array($res))
{
    $bookedby=isset($row['bookedby'])? $row["bookedby"] : NULL; 
}
}
?>
...
<script defer>
  ...
  show : () => {
    ...
    .then(data => { for (let r of data) {
      ...
      row.className = "row";
      row.innerHTML = 
        `<div class="title">[${r.track_time}] <br> NAME ${r.name}</div>
